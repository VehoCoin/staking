...
module.exports = async (deployer, network, accounts) => {
  ...
  const stakingContract = await PartnersStaking.at(createdStaking);
  ...
  await stakingContract.setRewards(web3.utils.toWei("10"), fromBlock, "50000", {
    from: accounts[1],
  });
  ...
  await devToken.approve(stakingContract.address, await ether("100000"), {
    from: accounts[1],
  });
  await stakingContract.stake(await ether("100"), { from: accounts[1] });

  console.log("Address Registry Staking contract: ", registry.address);
  console.log("Address Staking contract: ", stakingContract.address);
